...
# ------------------------------------------------------------------------
# 4. Spreadsheet CSV Import Dialog (LibreOffice/Excel Style)
# ------------------------------------------------------------------------
...
# ------------------------------------------------------------------------
# 5. Canvas Hover Tooltip System for Listbox
# ------------------------------------------------------------------------
...
class AdvancedAnalysisCanvas:
    ...
    def _apply_event_bindings(self):
        ...
        # Right-Click specifically requests context menu
    ...
    def _on_hover(self, e):
        ...
        for d_id, trace in {**self.datasets, **self.analysis_layers}.items():
            ...
            idx = max(0, min(np.searchsorted(trace["x"], hx), len(trace["x"]) - 1))
            if abs(trace["x"][idx] - hx) < min_delta_x:
                min_delta_x = abs(trace["x"][idx] - hx)
                closest = (trace["trace_name"], trace["x"][idx], trace["y"][idx], trace["color"])
    ...
    def redraw(self):
        ...
        for group in [self.datasets, self.analysis_layers]:
            for d_id, trace in group.items():
                ...
                # O(log N) slicing for ultra-high performance rendering
...
class App:
    ...
    def _build_ui_shell(self):
        ...
        self.context_menu.add_command(label="⚙ Re-configure Plot", command=self._reconfigure_selected_line)
        ...
        git_sec = sec("GIT VERSION CONTROL")
        tk.Button(git_sec, text='📂 Set Workspace & Init Git', bg=Theme.PNL2, fg=Theme.FG, relief='flat', bd=0, font=('Segoe UI', 10, 'bold'), cursor='hand2', command=self._set_git_workspace).pack(fill='x', ipady=3, pady=2)
        tk.Button(git_sec, text='💾 Commit Current State', bg=Theme.PNL2, fg=Theme.FG, relief='flat', bd=0, font=('Segoe UI', 10), cursor='hand2', command=self._commit_git_workspace).pack(fill='x', ipady=3, pady=2)
        tk.Button(git_sec, text='📋 Show Git Status', bg=Theme.PNL2, fg=Theme.DIM, relief='flat', bd=0, font=('Segoe UI', 10), cursor='hand2', command=self._show_git_status).pack(fill='x', ipady=2, pady=2)
    ...
    # ------------------------------------------------------------------
    def _set_git_workspace(self):
        if not GitEngine.is_git_installed():
            messagebox.showerror("Git Error", "Git is not installed or not in the system PATH.")
            return

        folder = filedialog.askdirectory(title="Select Git Workspace Directory")
        if not folder: return
        self.workspace_dir = folder
        
        success, out = GitEngine.init_repo(self.workspace_dir)
        if success:
            messagebox.showinfo("Git Init", f"Initialized Git repository in:\n{self.workspace_dir}\n\nOutput:\n{out}")
        else:
            messagebox.showerror("Git Init Failed", f"Failed to initialize repo:\n{out}")

    def _commit_git_workspace(self):
        if not GitEngine.is_git_installed():
            messagebox.showerror("Git Error", "Git is not installed or not in the system PATH.")
            return
        ...
        state = {
            ...
            "datasets": [v["filename"] for v in self.global_datasets.values()],
            "math_layers": list(self.global_math.keys()),
            "chart_configs": self.chart_configs
        }

        try:
            state_file = os.path.join(self.workspace_dir, "workspace_state.json")
            with open(state_file, 'w') as f:
                json.dump(state, f, indent=4)
        except Exception as e:
            messagebox.showerror("Save Error", f"Failed to save workspace state:\n{str(e)}")
            return

        msg = simpledialog.askstring("Git Commit", "Enter commit message:", initialvalue=f"Update workspace state {datetime.now().strftime('%Y-%m-%d %H:%M')}")
        if not msg: return

        success, out = GitEngine.commit_state(self.workspace_dir, msg)
        if success:
            messagebox.showinfo("Git Commit", f"Successfully committed workspace state.\n\nOutput:\n{out}")
        else:
            messagebox.showerror("Git Commit Failed", f"Failed to commit:\n{out}")
    ...
    def _show_git_status(self):
        ...
        success, out = GitEngine.get_status(self.workspace_dir)
        if success:
            messagebox.showinfo("Git Status", f"Repository: {self.workspace_dir}\n\n{out}")
        else:
            messagebox.showerror("Git Status Error", f"Failed to get status:\n{out}")
