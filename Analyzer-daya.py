...
class SpeechBubbleAnnotation(pg.GraphicsObject):
    ...
    def __init__(self, x, y, title, text, start_timestamp, delete_cb, edit_cb, select_cb):
        ...
        # UI Colors matched to Blue Theme
    ...
    def update_html_content(self):
        ...
        <div style="font-family: 'Segoe UI'; padding: 2px;">
            <div style="color: #1A73E8; font-size: 14px; font-weight: bold;">{self.title}</div>
    ...
    def paint(self, p, option, widget):
        ...
        if self.is_selected:
            p.setBrush(QColor(26, 115, 232, 20)) # Soft Blue
            p.setPen(pg.mkPen(color='#1A73E8', width=1.5, style=Qt.DashLine))
...
# ==========================================
# BEAUTIFUL TEXT EDITOR DIALOG (Blue Theme)
# ==========================================
class AnnotationDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLabel { font-family: 'Segoe UI'; font-weight: bold; color: #1A73E8; font-size: 11pt; margin-top: 4px; }
            QLineEdit, QTextEdit { 
            ...
            QLineEdit:focus, QTextEdit:focus { border: 2px solid #1A73E8; padding: 9px; }
            QPushButton { 
            ...
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if match:
                        ...
                        if len(cols) >= 3:
                            try:
                                float(cols[0].replace('\x00', '').replace('\ufeff', ''))
                                valid_rows += 1
            ...
            with open(self.file_path, "r", encoding='utf-8', errors='ignore') as f:
                ...
                while self.running:
                    ...
                    for line in lines:
                        ...
                        try:
                            # Primary Y1 Extraction
                            clean_val = cols[0].replace('\x00', '').replace('\ufeff', '')
                            y_val = float(clean_val) / 1000.0
                            ...
                            # Y2 Extraction (Temperature Kelvin to Celsius)
                            t_val = np.nan
                            if len(cols) >= 8:
                                try:
                                    temp_k = float(cols[7].replace('\x00', '').replace('\ufeff', ''))
                                    t_val = temp_k - 273.15
                                except ValueError:
                                    pass
...
# ==========================================
# MAIN APPLICATION GUI (Blue Theme)
# ==========================================
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MNST Analyzer IPU")
    ...
    def setup_ui(self):
        ...
        # --- NEW MAIN TITLE (Editable so you can rename the session) ---
        self.main_session_title = QLineEdit("Analyzer")
        ...
        lbl_stats_title.setStyleSheet("color: #1A73E8;") # Blue
        ...
        lbl_labels_title.setStyleSheet("color: #1A73E8;") # Blue
        ...
        input_style = """
            ...
            QDateTimeEdit:focus, QDoubleSpinBox:focus, QLineEdit:focus { border: 1px solid #1A73E8; background-color: #FFFFFF; }
        """

        self.title_edit = QLineEdit("Sensor Plot")
        self.xaxis_edit = QLineEdit("Time")
        ...
        self.chk_y2 = QCheckBox("Enable Y2 Axis(Temp °C)")
        ...
        lbl_delay_title.setStyleSheet("color: #1A73E8;") # Blue
        ...
        lbl_scale_title.setStyleSheet("color: #1A73E8;") # Blue
        ...
        btn_apply_scale.setStyleSheet("QPushButton { background-color: #F1F3F4; color: #1A73E8; padding: 8px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #E8F0FE; }")
        ...
        btn_auto_scale.setStyleSheet("QPushButton { background-color: #1A73E8; color: white; padding: 8px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #174EA6; }")
        ...
        # Blue highlighted readout
        self.live_emf_display = QLabel("0.000 mV")
        self.live_emf_display.setFont(QFont("Segoe UI", 14, QFont.Bold))
        self.live_emf_display.setStyleSheet("background-color: #E8F0FE; color: #1A73E8; padding: 6px 14px; border-radius: 8px; border: 1px solid #D2E3FC;")
        ...
        self.btn_load.setStyleSheet("QPushButton { background-color: #1A73E8; color: white; padding: 8px 14px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #174EA6; }")
        self.btn_load.setCursor(QCursor(Qt.PointingHandCursor))
        self.btn_load.clicked.connect(self.load_file)
        top_bar.addWidget(self.btn_load)

        self.btn_pause = QPushButton("Pause")
        self.btn_pause.setStyleSheet("QPushButton { background-color: #EA4335; color: white; padding: 8px 14px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #D93025; }")
        ...
        self.btn_package.setStyleSheet("QPushButton { background-color: #F29900; color: white; padding: 8px 14px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #E27200; }")
        ...
        # Dual Axis Setup (Y2 for Temperature)
        self.p2 = pg.ViewBox()
        ...
        self.graphWidget.getAxis('right').setLabel('Temperature (°C)', color='#EA4335', **{'font-size': '11pt', 'bold': True})
        ...
        # PRIMARY Y1 PLOT (EMF) - BLUE
        self.plot_curve = self.graphWidget.plot(
            pen=pg.mkPen('#1A73E8', width=2),
            symbol='o', symbolSize=6, symbolBrush='#1A73E8', symbolPen=None, 
            autoDownsample=True, downsampleMethod='subsample', clipToView=True
        )
        
        # SECONDARY Y2 PLOT (Temp) - RED
        self.plot_curve_temp = pg.PlotCurveItem(
            pen=pg.mkPen('#EA4335', width=2, style=Qt.DashLine),
            symbol='t', symbolSize=6, symbolBrush='#EA4335', symbolPen=None,
            autoDownsample=True, clipToView=True
        )
        self.p2.addItem(self.plot_curve_temp)
        
        # Crosshairs - GREEN
        self.vLine = pg.InfiniteLine(angle=90, movable=False, pen=pg.mkPen('#34A853', width=1.5, style=Qt.DashLine))
        self.hLine = pg.InfiniteLine(angle=0, movable=False, pen=pg.mkPen('#34A853', width=1.5, style=Qt.DashLine))
    ...
    def update_graph_labels(self):
        ...
        self.graphWidget.setLabel('left', self.yaxis_edit.text(), **{'color': '#1A73E8', 'font-size': '11pt', 'bold': True})
    ...
    def toggle_mouse_mode(self):
        ...
        if view_box.state['mouseMode'] == pg.ViewBox.RectMode:
            ...
            self.btn_mouse_mode.setStyleSheet("QPushButton { background-color: #E8F0FE; color: #1A73E8; padding: 8px 14px; border-radius: 6px; border: 1px solid #D2E3FC; font-weight: bold; }")
    ...
    def toggle_pause(self):
        ...
        if self.paused:
            ...
            self.btn_pause.setStyleSheet("QPushButton { background-color: #34A853; color: white; padding: 8px 14px; border-radius: 6px; font-weight: bold; } QPushButton:hover { background-color: #2D9248; }")
        else:
        ...
        if self.graphWidget.sceneBoundingRect().contains(pos):
            ...
            if idx < self.data_len:
                ...
                if self.y2_enabled and not np.isnan(exact_t):
                    self.tooltip_label.setText(f" {time_str} | Y1: {exact_y:.3f} mV | Temp: {exact_t:.1f} °C ")
                else:
